 ...
 */
#define CUR_FILE_VERSION \
  8 /* current file version; also the current AMX version */
#define MIN_FILE_VERSION \
  6 /* lowest supported file format version for the current AMX version */
#define MIN_AMX_VERSION \
  8 /* minimum AMX version needed to support the current file format */
...
typedef int(AMXAPI *AMX_CALLBACK)(struct tagAMX *amx, cell index, cell *result,
                                  cell *params);
...
typedef struct tagAMX {
  unsigned char _FAR *base PACKED; /* points to the AMX header plus the code,
                                      optionally also the data */
  unsigned char _FAR *data
      PACKED; /* points to separate data+stack+heap, may be NULL */
